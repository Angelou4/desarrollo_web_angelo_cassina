...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Extraemos las regiones (sus nombres) del nuevo objeto
    let regiones = regionComunaMap.regiones;
    let nombresRegiones = regiones.map(function(r) {
        return r.nombre;
    });
    cargarOpciones(regionSelect, nombresRegiones);

    regionSelect.addEventListener("change", function() {
        comunaSelect.innerHTML = "";
        let regionSeleccionada = regionSelect.value;

        // Buscamos la región correspondiente al nombre seleccionado
        let regionEncontrada = regiones.find(function(r) {
            return r.nombre === regionSeleccionada;
        });
        if (regionEncontrada) {
            // Extraemos el nombre de las comunas
            let nombresComunas = regionEncontrada.comunas.map(function(c) {
                return c.nombre;
            });
            cargarOpciones(comunaSelect, nombresComunas);
        }
    });
});